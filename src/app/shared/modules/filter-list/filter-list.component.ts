...
export class FilterList implements AfterViewInit {
  ...
  private readonly _itemTemplateDirective: TemplateDirective;

  ngAfterViewInit(): void {
    setTimeout(
      () => (this._itemTemplate = this._itemTemplateDirective?.template)
    );
  }
  ...
  public filter(value: string): any[] {
    ...
    if (value && this.hasItems && this.keywordFields) {
      const keywordFields: string[] = Array.isArray(this.keywordFields)
        ? this.keywordFields
        : [this.keywordFields];
